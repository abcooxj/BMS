...
void BMS_SysInitialize(void)
{
	...
	InitData.AlertOps.ocd 	 = BMS_ProtectHwOCD;//过流放电
	InitData.AlertOps.scd 	 = BMS_ProtectHwSCD;//短路放电
	InitData.AlertOps.ov	 = BMS_ProtectHwOV;//过压报警
	InitData.AlertOps.uv 	 = BMS_ProtectHwUV;	//欠压报警
	...
	InitData.ConfigData.UVPThreshold = INIT_UV_PROTECT * 1000;//欠压保护阈值
	InitData.ConfigData.OVPThreshold = INIT_OV_PROTECT * 1000;//过压保护阈值
	...
	// 软件初始化
	BMS_MonitorInit();	// 电池监控初始化		5
	BMS_ProtectInit();	// 电池保护初始化		4
	BMS_AnalysisInit();	// 电池分析初始化		3
	BMS_EnergyInit();	// 能量管理初始化			2
	BMS_InfoInit();		// 信息管理初始化   		1
