...
{
	.gpiox = I2C1_SCL_GPIO_Port,//SCL/SDA的引脚端口
	.sda_gpio_pin = I2C1_SDA_Pin,//SDA引脚
	.scl_gpio_pin = I2C1_SCL_Pin,//SCL引脚
	.retries = 3,//地址发送失败的重试次数
	.udelay = (void (*)(uint32_t))delay_us,
	.lockInit = I2C1_LockInit,//互斥锁初始化
	.lock = I2C1_Lock,//总线加锁，关闭中断
	.unlock = I2C1_Unlock,//总线解锁，使能中断
};
...
static inline uint8_t GET_SDA(struct I2C_BusTypeDef *bus)//读取当前电平
{
	return HAL_GPIO_ReadPin(bus->gpiox, bus->sda_gpio_pin);
}

static inline void SDA_L(struct I2C_BusTypeDef *bus)//拉低
{
    HAL_GPIO_WritePin(bus->gpiox, bus->sda_gpio_pin, GPIO_PIN_RESET);
}

static inline void SDA_H(struct I2C_BusTypeDef *bus)//拉高
{
    HAL_GPIO_WritePin(bus->gpiox, bus->sda_gpio_pin, GPIO_PIN_SET);
}

static inline void SCL_L(struct I2C_BusTypeDef *bus)//拉低
{
    HAL_GPIO_WritePin(bus->gpiox, bus->scl_gpio_pin, GPIO_PIN_RESET);
}

static inline void SCL_H(struct I2C_BusTypeDef *bus)//拉高
{
    HAL_GPIO_WritePin(bus->gpiox, bus->scl_gpio_pin, GPIO_PIN_SET);
}
